...
export function arenaWaitingText(
  ...
  if (!player1) {
    return (
      ...
      `Не отправляйте /start_tpd_arena повторно — иначе откроется другая арена.` +
      desktopOnly
    );
  }
  return (
...
export function arenaAlreadyOpenText(arena: {
  ...
  return (
    ...
    (p1 ? `Левый боец: ${p1}\n` : "") +
    `Ждём правого бойца…\n\n` +
    `Не отправляйте /start_tpd_arena снова — оба бойца нажимают кнопку в сообщении с этим кодом.` +
    desktopOnly
  );
}
